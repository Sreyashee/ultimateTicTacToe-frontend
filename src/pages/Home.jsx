...
const Home = () => {
  ...
  const [isCodeGenerated, setIsCodeGenerated] = useState(false);
  const [isConnecting, setIsConnecting] = useState(false);
  const [nextRoute, setNextRoute] = useState(null); // NEW
  ...
  const handleUltimate = () => {
    if (!name.trim()) {
      setNextRoute('/ultimate');
      setShowPopup(true);
    } else {
      navigate('/ultimate', { state: { name } });
    }
    setError('');
  };
  ...
  const handleGenerateCode = () => {
    ...
    socket.emit('createRoom', { roomCode: newCode, name }, (response) => {
      if (response.error) {
        alert(response.error);
        setIsConnecting(false);
      } else {
  ...
  const handleJoinRoom = () => {
    ...
    socket.emit('joinRoom', { roomCode: normalizedCode, name }, (response) => {
      if (response.error) {
        alert(response.error);
        setIsConnecting(false);
      }
  ...
  return (
    <div className="home-container">
      ...
      <div className="box ultimate">
        <h1>Ultimate TicTacToe</h1>

        <button onClick={handleUltimate}>Ultimate TicTacToe</button>
      ...
      {showMultiplayer && (
        <div className="modal-overlay">
          <div className="popup modal-popup">
            <button className="close-btn" onClick={() => setShowMultiplayer(false)} disabled={isConnecting}>×</button>
            <h3>Enter your name</h3>
            <input
              className="name-input"
              value={name}
